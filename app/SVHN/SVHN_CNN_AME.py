...
from SVHN_CNN import main, get_filename
...
#%%
    ...
    parser.add_argument('--epoch_refine', default=80, type=int)
    parser.add_argument('--epoch_freeze', default=80, type=int)
    ...
    #%%
    #-------------------------------------------
    sample_count_train=sample_count_train=len(loader_check.dataset)
    
    os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"   # see issue #152
    os.environ["CUDA_VISIBLE_DEVICES"]=str(arg.cuda_id)
    device=torch.device('cuda' if torch.cuda.is_available() else "cpu")
    arg.delta=np.ceil((1/arg.epoch_end)/(1/255))*1/255
