...
class AutoAttack(AutoAttack):
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=False,
        ...
        super().__init__(model, norm=norm, eps=eps, seed=seed, verbose=verbose,
                 attacks_to_run=attacks_to_run, version=version, is_tf_model=is_tf_model,
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        n_cls = checks.check_range_output(self.get_logits, x_orig[:bs].to(self.device),
            logger=self.logger)
        # the number of targeted class should be at most nclasses - 1
        # https://github.com/fra31/auto-attack/issues/76
        self.apgd_targeted.n_target_classes = n_cls - 1
        self.fab.n_target_classes = n_cls - 1
        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
